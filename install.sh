...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        curl -4fLRo ${xui_folder}-linux-$(arch).tar.gz https://github.com/${author}/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        url="https://github.com/${author}/3x-ui/releases/download/${tag_version}/x-ui-linux-$(arch).tar.gz"
    ...
    curl -4fLRo /usr/bin/x-ui-temp https://raw.githubusercontent.com/${author}/3x-ui/main/x-ui.sh
    if [[ $? -ne 0 ]]; then
    ...
    if [[ $release == "alpine" ]]; then
        curl -4fLRo /etc/init.d/x-ui https://raw.githubusercontent.com/${author}/3x-ui/main/x-ui.rc
        if [[ $? -ne 0 ]]; then
        ...
        if [ "$service_installed" = false ]; then
            ...
            case "${release}" in
                ubuntu | debian | armbian)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/${author}/3x-ui/main/x-ui.service.debian > /dev/null 2>&1
                    ;;
                arch | manjaro | parch)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/${author}/3x-ui/main/x-ui.service.arch > /dev/null 2>&1
                    ;;
                *)
                    curl -4fLRo ${xui_service}/x-ui.service https://raw.githubusercontent.com/${author}/3x-ui/main/x-ui.service.rhel > /dev/null 2>&1
                    ;;
            esac
